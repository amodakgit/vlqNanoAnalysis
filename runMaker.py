...
options.register('pyInput', 'test',
                 VarParsing.multiplicity.singleton,
                 VarParsing.varType.string,
                 "path to read config"
)

options.register('output', '/store/user/amodak/NanoPostProOut', 
#options.register('output', '/store/user/amodak/Skim/2017', 
                 VarParsing.multiplicity.singleton,
                 VarParsing.varType.string,
                 "path to output"
)
options.register('test', '/uscms/homes/a/amodak/work/CMSSW_8_0_26/src/Analysis/VLQAna/test', 
                 VarParsing.multiplicity.singleton,
                 VarParsing.varType.string,
...
for year in range(len(years)):
    ...
    files =[]
    #if (str(years[year]) == "2016"): files = files_2016
    #elif (str(years[year]) == "2017"): files = files_2017
    #elif (str(years[year]) == "2018"): files = files_2018
    ...
    with open(files, 'r') as f:
     for content in f:
       ...
       outputFile = open('run_'+str(log)+'.sh', 'w')
       for line in inputFile:
         if 'INFILE' in line :
           line = line.replace('INFILE', infile)
         if 'OUTFILE' in line:
           line = line.replace("\n", " ")
           line = line.replace('OUTFILE', outfile)
         if 'YEAR' in line:
           line = line.replace("\n", " ")
           line = line.replace('YEAR', str(years[year]))
         if 'OUTPUT' in line:
           line = line.replace('OUTPUT', output+"/"+str(years[year]))
           line = line.replace("\n", " ")
